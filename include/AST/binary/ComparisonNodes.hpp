...
public:                                                                               \
    ...
    {                                                                                 \
        ...
        auto type = typing_system().get_winning_type(lhs_value.type, rhs_value.type); \
...
COMPARISON_EXP_NODE(EQ, CreateICmpEQ, CreateFCmpUEQ, "eq_value")
    ...
    class NENode : public ASTNode {
        ...
        ASTNode *rhs;
    public:
        NENode(ModuleCompiler *compiler, ASTNode *lhs, ASTNode *rhs) : lhs(lhs), rhs(rhs) { this->compiler = compiler; }
        static Value perform(ModuleCompiler *compiler, const Value &lhs, const Value &rhs, const Type *type) {
            auto l = lhs.cast_as(type, false);
            auto r = rhs.cast_as(type, false);
            if (l.is_null() || r.is_null())
                report_error("Can't perform the operation " "NE" " between the types " + lhs.type->to_string() + " and " +
                             rhs.type->to_string());
            llvm::Value *ptr;
            if (type->as<FloatType>()) {
                ptr = compiler->get_builder()->CreateFCmpUNE(l.get(), r.get(), "temp_ne");
            }
            else { ptr = compiler->get_builder()->CreateICmpNE(l.get(), r.get(), "temp_ne"); }
            auto value = compiler->create_value(ptr, type);
            return value.cast_as(compiler->create_type<I8Type>());
        }
        Value eval() override {
            ...
            return NENode::perform(compiler, lhs_value, rhs_value, type);
        }
        const Type *get_type() override {
            if (compiler->clear_type_cache)type = nullptr;
            if (type != nullptr)return type;
