...
std::array<LexRule, Tokenizer::LEX_RULES_COUNT> Tokenizer::lexRules_ = {{
  ...
  {std::regex(R"(^\/\*[\s\S]*?\*\/)"), &_lexRule5},
  {std::regex(R"(^\d+)"), &_lexRule6},
  {std::regex(R"(^[\w\_\$]+)"), &_lexRule7},
...
auto __ = Expression(SExpressionType::IDENTIFIER, _1) ;
...
auto __ = Expression(SExpressionType::SYMBOL, _1) ;
...
auto __ = Expression(SExpressionType::STRING, _1.substr(1, _1.size() - 2)) ;
...
auto __ = Expression(std::stoll(_1)) ;
...
auto __ = Expression(std::vector<Expression>()) ;
